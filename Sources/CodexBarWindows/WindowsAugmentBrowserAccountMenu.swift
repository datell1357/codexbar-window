...
enum WindowsAugmentBrowserAccountMenu {
    static func choose(owner: HWND, rows: [WindowsUsageRuntime.AugmentBrowserChoice],
                       failedCount: Int, omittedCount: Int) -> UUID? {
        guard !rows.isEmpty, rows.count <= 16, let menu = CreatePopupMenu() else { return nil }
        defer { DestroyMenu(menu) }
        let notice = "Choose a verified Firefox session (unavailable: \(failedCount), not checked: \(omittedCount))"
