#if os(Windows)
// Preserve original share aggregation, sanitization and partial-coverage semantics.
import CodexBarCore
import Foundation

struct WindowsShareStatsProviderPayload: Sendable, Equatable {
...
struct WindowsShareStatsModelPayload: Sendable, Equatable {
    let provider: UsageProvider
    let providerName: String
...
private struct WindowsShareStatsModelFamilyKey: Hashable {
    let provider: UsageProvider
    let providerName: String
    let modelName: String
    let currencyCode: String
...
private struct WindowsShareStatsModelFamilyAccumulator {
    let key: WindowsShareStatsModelFamilyKey
    ...
    init(key: WindowsShareStatsModelFamilyKey, row: WindowsShareStatsModelPayload) {
        self.key = key
    ...
    mutating func add(_ row: WindowsShareStatsModelPayload) {
    ...
    var payload: WindowsShareStatsModelPayload? {
...
struct WindowsShareStatsCurrencyPayload: Sendable, Equatable, Identifiable {
    let currencyCode: String
    let estimatedCost: Double?
    let coveredDayCount: Int
    let isPartial: Bool
...
struct WindowsShareStatsPayload: Sendable, Equatable {
    let days: Int
    let periodEnd: Date
    let providers: [WindowsShareStatsProviderPayload]
    let topModels: [WindowsShareStatsModelPayload]
    let currencies: [WindowsShareStatsCurrencyPayload]
    let totalTokens: Int?
    ...
    init(
        days: Int,
        periodEnd: Date,
        providers: [WindowsShareStatsProviderPayload],
        topModels: [WindowsShareStatsModelPayload],
        currencies: [WindowsShareStatsCurrencyPayload],
        totalTokens: Int?,
...
struct WindowsShareStatsSubscriptionName: Sendable, Equatable {
...
enum WindowsShareStatsSanitizer {
...
enum WindowsShareStatsBuilder {
    static func make(
        model: WindowsSpendDashboardModel,
        subscriptionNames: [String: WindowsShareStatsSubscriptionName] = [:]) -> WindowsShareStatsPayload?
    {
        ...
        let sanitizedModels = model.groups.filter {
            ...
            group.models.compactMap { row -> WindowsShareStatsModelPayload? in
                let estimatedCost = self.finiteCost(row.totalCost)
                guard let modelName = WindowsShareStatsSanitizer.modelName(row.modelName),
                      row.totalTokens != nil
                else { return nil }
                return WindowsShareStatsModelPayload(
                    provider: row.provider,
        ...
        var modelFamilies: [WindowsShareStatsModelFamilyKey: WindowsShareStatsModelFamilyAccumulator] = [:]
        for row in sanitizedModels {
            ...
            if var existing = modelFamilies[key] {
                ...
                modelFamilies[key] = WindowsShareStatsModelFamilyAccumulator(key: key, row: row)
            }
        }
        ...
        let currencies = model.groups.map {
            WindowsShareStatsCurrencyPayload(
        ...
        let payload = WindowsShareStatsPayload(
...
enum WindowsShareStatsFormatting {
    ...
    static func isAllTime(_ payload: WindowsShareStatsPayload) -> Bool {
        payload.days >= WindowsSpendHistoryPolicy.scanDays
    }

    static func periodHeadline(_ payload: WindowsShareStatsPayload) -> String {
        self.isAllTime(payload)
            ? "My AI subscriptions · all time"
            : "My AI subscriptions · last \(payload.days) days"
    }

    static func coverageFraction(covered: Int, payload: WindowsShareStatsPayload) -> String {
        self.isAllTime(payload)
            ? "\(covered)/all"
