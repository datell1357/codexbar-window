...
struct WindowsSpendSnapshotLoader {
    struct Source: Sendable, Equatable {
        ...
        var expectedAntigravityCredentialFingerprint: String? = nil
        /// Supplied from a successful fetch in the current runtime generation, never stale tray state.
        var providerProjection: WindowsSpendProviderProjection? = nil
        ...
        var supportsRetainedCollection: Bool {
            ...
            case .antigravity: self.expectedAntigravityCredentialFingerprint != nil
            default: false
            }
        }
    }
    enum Failure: Error { case invalidSources, missingCodexHome, codexOwnerChanged, claudeOwnerChanged, vertexOwnerChanged, antigravityOwnerChanged, providerSnapshotUnavailable }
        ...
        { days in
            ...
            for source in sources {
                ...
                do {
                    ...
                    if source.provider == .antigravity, let expected = source.expectedAntigravityCredentialFingerprint {
                        guard AntigravityOAuthCredentialsStore.credentialFileFingerprint(environment: environment) == expected
                        else { throw Failure.antigravityOwnerChanged }
                    }
                    ...
                    if WindowsSpendProviderProjection.supports(source.provider) {
                        guard let projection = source.providerProjection, projection.provider == source.provider
                        else { throw Failure.providerSnapshotUnavailable }
                        snapshot = try projection.snapshot(historyDays: historyDays)
                    } else {
                        snapshot = try await fetcher.loadTokenSnapshot(provider: source.provider,
                    ...
                    if source.provider == .antigravity, let expected = source.expectedAntigravityCredentialFingerprint,
                       AntigravityOAuthCredentialsStore.credentialFileFingerprint(environment: environment) != expected {
                        throw Failure.antigravityOwnerChanged
                    }
                       ...
                       WindowsWidgetConfiguration.selectableProviders.contains(source.provider) {
                        do {
                            ...
                            if source.provider == .antigravity {
                                guard let expected = source.expectedAntigravityCredentialFingerprint,
                                      AntigravityOAuthCredentialsStore.credentialFileFingerprint(environment: environment) == expected
                                else { throw Failure.antigravityOwnerChanged }
                            }
                            ...
                            switch error {
                            case WindowsWidgetCostAdapter.Failure.scopeChanged, Failure.codexOwnerChanged,
                                 Failure.claudeOwnerChanged, Failure.vertexOwnerChanged, Failure.antigravityOwnerChanged:
                    ...
                    let identityUnconfirmed = error is CursorCostAccountIdentityError
                        ...
                        || ownerFailure == .vertexOwnerChanged || ownerFailure == .antigravityOwnerChanged
