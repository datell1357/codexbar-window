    ...
    static func credentialIssue(provider: UsageProvider?, token: String) -> String? {
        if provider == .zed {
            do { try ZedManualCredentialInput.validate(token); return nil }
                ...
                return "Enter a positive numeric Zed user ID, one space, and its access token. Optionally append one space and an HTTPS server origin without a path, query or login information. The input must fit within 64 KiB."
            }
        }
