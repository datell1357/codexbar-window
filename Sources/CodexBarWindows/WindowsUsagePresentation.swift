...
public struct WindowsUsagePresentation: Sendable {
    ...
    public func rows(now: Date = Date()) -> [String] {
        ...
        if let credits = self.result?.credits, presentation?.menuCard.showsCreditsSection != false, self.showOptionalUsage {
            ...
            if let limit = credits.codexCreditLimit {
                rows.append("\(limit.title): \(UsageFormatter.percentString(limit.remainingPercent)) left")
                if let reset = limit.resetsAt {
                    ...
                    if let text = UsageFormatter.resetLine(for: resetWindow, style: .countdown, now: now) { rows.append(text) }
                }
            }
        }
        if let presentation {
    ...
    private static func rateRow(label: String, window: RateWindow, now: Date, detailAsReset: Bool = false, usageKnown: Bool = true) -> String {
        ...
        let reset = detailAsReset ? window.resetDescription : UsageFormatter.resetLine(for: window, style: .countdown, now: now)
