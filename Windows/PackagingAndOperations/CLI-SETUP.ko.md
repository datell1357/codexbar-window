...
앱은 `-ResultProtocolV1`로 helper를 호출한다. 이 내부 프로토콜의 정상 종료 코드는20(쓰기 성공),21(이미 요청 상태),22(ShouldProcess 거부/미리보기)이며0을 포함한 다른 코드는 앱에서 성공으로 간주하지 않는다. 일반 수동 호출에서는 기존 PowerShell 출력과 성공 종료 관례를 유지한다. 원시 PATH나 오류 문자열을 앱으로 전달하지 않는다.

앱 호출에서20이 반환되면 WM_SETTINGCHANGE/Environment 통지를 시도한다. 수신 앱의 갱신 완료는 보장하지 않으며 이미 열린 프로세스 환경은 교체하지 않는다.100ms timeout은 수신 창마다 적용되므로 전체 통지 시간의 상한이 아니다. 수동 호출의 로그아웃·로그인 안내는 유지한다.
