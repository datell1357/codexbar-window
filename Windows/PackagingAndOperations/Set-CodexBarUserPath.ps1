...
try {
    ...
    if ($Action -eq 'Add') {
        if ($matching.Count -gt 0) { Write-Output 'Already present; user Path unchanged.'; return }
        ...
        if ($matching.Count -eq 0) { Write-Output 'No matching literal entry; user Path unchanged.'; return }
    ...
    if (-not $PSCmdlet.ShouldProcess('Current user Path', "$Action the specified CLI directory")) { return }
